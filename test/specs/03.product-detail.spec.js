...
describe('03. 상품 상세', () => {
  ...
  it('[TC-PDETAIL-007] 수량 1에서 - 버튼 클릭 시 0으로 감소 (경계값)', async () => {
    await ProductDetailPage.decreaseQuantity();

    await expect(await ProductDetailPage.getQuantity()).toBe('0');
  });

  it('[TC-PDETAIL-008] 장바구니 담기 후 장바구니 아이콘이 노출됨', async () => {
